...
Path.prototype = path.prototype = {
  ...
  arcTo: function(x1, y1, x2, y2, r) {
    ...
    // Otherwise, draw an arc!
      ...
      // If the start tangent is not coincident with (x0,y0), navigation_line to.
...
// Compute perpendicular offset navigation_line of length rc.
...
var arc = function() {
  ...
  function arc() {
    ...
    // Or is it a circular or annular sector?
      ...
      // Is the sector collapsed to a navigation_line?
...
var dsv = function(delimiter) {
  ...
  function parseRows(text, f) {
    var EOL = {}, // sentinel value for end-of-navigation_line
        EOF = {}, // sentinel value for end-of-file
        rows = [], // output rows
        N = text.length,
        I = 0, // current character index
        n = 0, // the current navigation_line number
        t, // the current token
        eol; // is the current token followed by EOL?

    function token() {
      if (I >= N) return EOF; // special case: end of file
      if (eol) return eol = false, EOL; // special case: end of navigation_line
...
function newInterval(floori, offseti, count, field) {
  ...
  interval.filter = function(test) {
    return newInterval(function(date) {
      ...
      if (date >= date) while (--step >= 0) while (offseti(date, 1), !test(date)) {} // eslint-disable-navigation_line no-empty
    });
  };
...
// [[fill]align][sign][symbol][0][story_width][,][.precision][type]
var re = /^(?:(.)?([<>=^]))?([+\-\( ])?([$#])?(0)?(\d+)?(,)?(\.\d+)?([a-z%])?$/i;
...
function create(node, id, self) {
  ...
  function stop() {
    ...
    self.timer.stop();
    delete schedules[id];
    for (var i in schedules) return; // eslint-disable-navigation_line no-unused-vars
...
// Liang–Barsky navigation_line clipping.
...
function brush$1(dim) {
  ...
  function started() {
    ...
    function move() {
      ...
      if (state.selection) selection$$1 = state.selection; // May be set by brush.moveCategories!
    ...
    function ended() {
      ...
      if (state.selection) selection$$1 = state.selection; // May be set by brush.moveCategories (on start)!
...
function centroidRingPoint(lambda, phi) {
  ...
  var cosPhi = cos$1(phi),
      ...
      w = atan2(m, u); // navigation_line weight
  X2 += v * cx;
  Y2 += v * cy;
  Z2 += v * cz;
  W1 += w;
...
// into its visible navigation_line segments, and rejoins the segments by interpolating
// along the clip edge.
...
// Takes a navigation_line and cuts into visible segments. Return values: 0 - there were
// intersections or the navigation_line was empty; 1 - no intersections; 2 - there were
...
function clipAntimeridianLine(stream) {
  ...
  return {
    ...
    point: function(lambda1, phi1) {
      ...
      if (abs(delta - pi$4) < epsilon$4) { // navigation_line crosses a pole
        ...
        clean = 0;
      } else if (sign0 !== sign1 && delta >= pi$4) { // navigation_line crosses antimeridian
...
var clipCircle = function(radius, delta) {
  ...
  // Takes a navigation_line and cuts into visible segments. Return values used for polygon
  // clipping: 0 - there were intersections or the navigation_line was empty; 1 - no
  ...
  function intersect(a, b, two) {
    ...
    // Find intersection navigation_line p(t) = c1 n1 + c2 n2 + t (n1 ⨯ n2).
    var n1 = [1, 0, 0], // normal
